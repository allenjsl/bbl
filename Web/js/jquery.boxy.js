...
jQuery.extend(Boxy, {
    ...
    //"<tr><td class='boxy-top-left'></td><td class='boxy-top'></td><td class='boxy-top-right'></td></tr>" +
                "<tr><td style='border:2px solid #007AA5' class='boxy-inner'></td></tr>" +
    ...
    iframeDialog: function(options) {
        ...
        if(winH<(iframeHeight+40)){
            iframeHeight = winH-40;
        }
        // If data is available, append data to url for get requests                 //src='"+options.iframeUrl+"'
        var body = jQuery("<iframe src='about:blank' name=" + iframeId + " id=" + iframeId + " style='margin:0px;padding:0px;' scrolling='" + options.scrolling + "' width='" + iframeWidth + "' height='" + iframeHeight + "'  frameborder='0'><\/iframe>");
...
Boxy.prototype = {
    ...
    _setupTitleBar: function() {
        if (this.options.title) {
            ...
            if (this.options.closeable) {
                tb.append(jQuery("<a href='#' class='close' hideFocus='true'></a>").html(this.options.closeText));
            }
