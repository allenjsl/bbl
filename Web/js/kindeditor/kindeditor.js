...
var keMore=['source','|','undo', 'redo','|','cut','copy', 'paste','plainpaste', 'wordpaste','|',
			 ...
			 'removeformat','|', 'link','unlink'];
var keMore_HaveImage=['source','|','undo', 'redo','|','cut','copy', 'paste','plainpaste', 'wordpaste','|',
             'justifyleft', 'justifycenter', 'justifyright','justifyfull','insertorderedlist', 'insertunorderedlist', 'indent', 'outdent', 'subscript',
		     'superscript','-','title','fontname', 'fontsize', 'textcolor', 'bgcolor', 'bold', 'italic', 'underline','advtable',
...
KE.plugin['image'] = {
        ...
        exec: function(id) {
            ...
            var height = KE.$('imgHeight', dialogDoc).value;
            var title = KE.$('imgTitle', dialogDoc).value;
            var vmargin = KE.$('imgvMargin', dialogDoc).value;
            var hmargin = KE.$('imghMargin', dialogDoc).value;
            ...
            if (type == 2) {
                ...
                var onloadFunc = function() {
                    ...
                    if (typeof data === 'object' && 'error' in data) {
                        if (data.error === 0) {
                            self.insert(id, data.url, title, width, height, vmargin, hmargin, 0, align);
                        } else {
                            alert(data.message);
                            return false;
                        }
                ...
                var url = KE.$('url', dialogDoc).value;
                this.insert(id, url, title, width, height, vmargin, hmargin, 0, align);
            }
        },
        insert: function(id, url, title, width, height, vmargin, hmargin, border, align) {
            ...
            if (vmargin) html += "margin-left:" + hmargin.replace("px", "") + "px;margin-right:" + hmargin.replace("px", "") + "px;";
            if (hmargin) html += "margin-top:" + vmargin.replace("px", "") + "px;margin-bottom:" + vmargin.replace("px", "") + "px;";
            if (align) html += "float:" + align + ";";
            html += '"';
            html += 'alt="' + title + '" ';
            html += 'border="' + border + '" />';
            KE.util.insertHtml(id, html);
