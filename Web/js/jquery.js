...
(function() {
    ...
    function evalScript(i, elem) {
        var cache = false;
        if (elem.getAttribute("cache") == "true") {
            cache = true;
        }
        if (elem.src)
            jQuery.ajax({
...
/*
jquery ajaxSettings xhr override
*/
/*
...
*/
jQuery.ajaxSettings.xhr = window.XMLHttpRequest && (window.location.protocol !== "file:" || !window.ActiveXObject) ?
			function() {
...
jQuery.newAjax = function(options) {
    ...
    if (isPostType) {
        if (options.url.indexOf('?') == -1) {
            options.url = options.url + "?urltype=pageajax";
        } else {
            options.url = options.url + "&urltype=pageajax";
        }
    } else {
        options.data = $.extend({ urltype: "pageajax" }, options.data);
    }
    var orisucess;
    if (options.success) {
