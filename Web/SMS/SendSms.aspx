<%@ Page Title="发送短信_短信中心" Language="C#" MasterPageFile="~/masterpage/Back.Master" AutoEventWireup="true" CodeBehind="SendSms.aspx.cs" Inherits="Web.SMS.SendSms" %>
   ...
   <div class="btnbox" style="height:5px;">
        ...
        <input type="hidden" name="method" value="valid" id="method" />
            	...
            	<table width="100%" border="0" cellpadding="0" cellspacing="1">
                  <tr>
                    <td width="14%"  align="right" bgcolor="#BDDCF4"><font color="#FF0000">*</font><strong>编辑发送内容：</strong></td>
                    <td width="86%" align="left" bgcolor="#e3f1fc" class="pandl4">
短信内容：当前短信共有 <span id="sNum">0</span> 字。移动分为 <span id="ysNum">0</span> 条发送，联通分为 <span id="lsNum">0</span> 条发送，小灵通分为 <span id="xsNum">0</span> 条发送<br />
<textarea name="textContent" cols="70" rows="5" onkeyup="SendSms.fontNum(this);" id="textContent"></textarea>
<br />
                    <font color="#FF0000">*注：移动、联通短信内容不能超过70字!小灵通短信内容不超过45个字</font><br />
					<a href="javascript:;"  onclick="return SendSms.autoSms();"><img src="/images/open-dx.gif" /><strong>自动填写发送内容 </strong></a><br />
</td>
                  </tr>
                  <tr>
                    <td  align="right" valign="top" bgcolor="#BDDCF4" class="pandl4"><font color="#FF0000">*</font><strong>手机号码：</strong></td>
                    <td align="left" bgcolor="#e3f1fc" class="pandl4"><textarea id="txtMobiles" cols="70" rows="5" ></textarea>
                    <br />
					...
					（只能导入格式为.xls和.txt的文件，号码只能一行一个）</td>
                  </tr>
                   <tr>
                    <td  align="right" bgcolor="#BDDCF4"><strong>发送方式：</strong></td>
                    <td align="left" bgcolor="#e3f1fc" class="pandl4">
                    <select name="selSendType" onchange="return SendSms.selSendType()">
                      <option value="0" selected="selected">直接发送</option>
                      <option value="1">定时发送</option>
                    </select> 
                    <input type="text" id="txtSendTime" name="txtSendTime" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm'})" style="display:none" />
                     定时短信不能取消                     </td>
                  </tr>
                  <tr>
                    <td  align="right" bgcolor="#BDDCF4"><strong>选择发送通道：</strong></td>
                    <td align="left" bgcolor="#e3f1fc" class="pandl4">
                    <select name="selChannel" runat="server" id="selChannel">
                     
                    </select></td>
                  </tr>
                   <tr>
                    <td  align="right" bgcolor="#BDDCF4">
                    <input type="checkbox"  id="chkSender" name="chkSender" value="hasSender" onclick="return SendSms.selSender(this);" />
                     <strong>发信人：</strong></td>
                    <td align="left" bgcolor="#e3f1fc" class="pandl4"><input name="txtSender" id="txtSender" type="text"  class="searchinput2"  size="35"/>
                     打勾后短信内容将包含发信人信息（发信人要占用内容字数） </td>
                  </tr>
                   <tr>
                     <td  align="right" bgcolor="#e3f1fc"><font color="#FFFFFF">&nbsp;</font></td>
                     <td align="left" bgcolor="#e3f1fc" class="pandl4"><table border="0" align="left" cellpadding="0" cellspacing="0">
                       <tr>
                         <td width="134" height="40" align="center" class="jixusave"><font color="#FFFFFF">
                         <input type="button" onclick="return SendSms.sendMess();" id="btnSend" value="提交并发送短信"   style="background-image:url(/images/jixusave.gif); border:none; width:134px; font-size:14px;  height:31px; cursor:pointer; font-weight:bold" />
                         </font></td>
                       </tr>
                     </table>
                     <div id="sendMess" style="color:Red; height:40px; vertical-align:middle;"></div>
                     </td>
                   </tr>
                   <tr>
                     <td  align="right" bgcolor="#BDDCF4">&nbsp;</td>
                     <td align="left" bgcolor="#BDDCF4" class="pandl4"><font color="#FF0000">
                     <asp:Literal ID="remainNum" runat="server"  Visible="false"></asp:Literal></font><br />
					   <a href="javascript:;"  onclick="return SendSms.recharge();" <%=showPay %> ><img src="/images/dx-chongzhi.gif" /></a> </td>
                   </tr>
              </table>
              </div>
              </form>
              ...
              <script type="text/javascript">
                  ...
                  var SendSms = {
                      ...
                      openDialog: function(p_url, p_title, p_width, p_height) {
                          Boxy.iframeDialog({ title: p_title, iframeUrl: p_url, width: p_width, height: p_height, data: "txtMobiles" }); return false;
                      },
                      //从文件导入号码
                      putInFile: function() {
