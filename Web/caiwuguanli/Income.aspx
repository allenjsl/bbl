<%@ Page Title="" Language="C#" MasterPageFile="~/masterpage/Back.Master" AutoEventWireup="true"
    CodeBehind="Income.aspx.cs" Inherits="Web.caiwuguanli.Income" %>

<%@ Register Assembly="ControlLibrary" Namespace="Adpost.Common.ExportPageSet" TagPrefix="cc1" %>
<%@Import Namespace="EyouSoft.Common" %>
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    <title>杂费收入_财务管理</title>
</asp:Content>
                        ...
                        <tr>
                            <td align="left" width="90">
                                <asp:Panel ID="pnlAdd" runat="server">
                                    <a id="btnAdd" href="javascript:void(0);">登 记</a>
                                </asp:Panel>
                            </td>
                        </tr>
                    </tbody>
                    ...
                    <tbody>
                        ...
                        <asp:Repeater ID="rptList" runat="server">
                            <ItemTemplate>
                                ...
                                <td align="center">
                                    <a href="javascript:void(0);" onclick="OpenDetails('<%#Eval("IncomeId")%>','income'); return false;">
                                        <font class="fblue">修改</font></a> <a href="javascript:void(0);" onclick="DeleteDetails('<%#Eval("IncomeId")%>','<%#Eval("tourId")%>')">
                                            <font class="fblue">删除</font></a> 
                                            <%if (CheckGrant(Common.Enum.TravelPermission.财务管理_杂费收入_收入审核)){%>
                                                <a href="javascript:void(0);" onclick="SetOtherCostStatus(this, '<%#Eval("Status")%>','<%#Eval("IncomeId")%>')"><font class="fblue"><%#(bool)Eval("Status") ? "取消收款" : "确认收款"%></font></a>
                                            <%} %>
                                </td>
                                </tr>
                            </ItemTemplate>
    ...
    <script type="text/javascript">
        $(function() {
            $("#btnSearch").click(function() {
                ...
                window.location.href = "/caiwuguanli/Income.aspx?" + $.param(para);
                return false;
            });
        ...
        function OpenDetails(id, type) {
            ...
            Boxy.iframeDialog({ title: "杂费收入", iframeUrl: "/sanping/Tianjiasr.aspx?type=Income&id=" + id, width: "510px", height: "430px", draggable: true, data: null, hideFade: true, modal: true });
        ...
        function DeleteDetails(id, tourId) {
            if (confirm("确定删除吗?")) {
                $.newAjax({
                    type: "POST",
                    url: "/caiwuguanli/AjaxIncome.ashx?id=" + id + "&tourId=" + tourId + "&type=income" + "&v=" + Math.random(),
                    cache: false,
        ...
        <%if (CheckGrant(Common.Enum.TravelPermission.财务管理_杂费收入_收入审核)){%>
        function SetOtherCostStatus(thiss, status, IncomeId) {
            if (confirm("确定" + $.trim($(thiss).find(".fblue").html()) + "吗?")) {
                $.newAjax({
                    type: "POST",
                    url: "/caiwuguanli/AjaxIncome.ashx?cat=status&IncomeId=" + IncomeId + "&status=" + status + "&verifyType=in",
                    cache: false,
                    success: function(state) {
                        if (state == "OK") {
