...
namespace EyouSoft.DAL.CompanyStructure
{
    public class CustomerCareFor:EyouSoft.Toolkit.DAL.DALBase,EyouSoft.IDAL.CompanyStructure.ICustomerCareFor
    {
        ...
        {
            StringBuilder sql = new StringBuilder("INSERT INTO [tbl_CustomerCarefor]([CompanyId],[MobileCode],[IsMatchCustomerInfo],[IsMatchSupplierInfo],[IsMatchDepartmentInfo],[Content],[Time],[FixType],[ChannelId],[IsEnabled],[OperatorId],[IssueTime])");
        ...
        public bool Update(EyouSoft.Model.CompanyStructure.CustomerCareforInfo Model)
        {
            if (Model == null || Model.Id <= 0)
                return false;
            StringBuilder sql = new StringBuilder("update [tbl_CustomerCarefor] set [CompanyId]=@CompanyId,[MobileCode]=@MobileCode,[IsMatchCustomerInfo]=@IsMatchCustomerInfo,");
        ...
        {
            string sql = " update tbl_CustomerCarefor set IsEnabled=0 where id=@CareForId ";
            DbCommand cmd = this._db.GetSqlStringCommand(sql);
        ...
        {
            string sql = " update tbl_CustomerCarefor set IsEnabled=1 where id=@CareForId ";
            DbCommand cmd = this._db.GetSqlStringCommand(sql);
            this._db.AddInParameter(cmd, "CareForId", DbType.Int32, CareForId);
            return DbHelper.ExecuteSql(cmd, this._db) > 0 ? true : false;
        }
        ...
        {
            string sql = " DELETE FROM tbl_CustomerCarefor  where id=@CareForId ";
            DbCommand cmd = this._db.GetSqlStringCommand(sql);
            this._db.AddInParameter(cmd, "CareForId", DbType.Int32, CareForId);
            return DbHelper.ExecuteSql(cmd, this._db) > 0 ? true : false;
        }
